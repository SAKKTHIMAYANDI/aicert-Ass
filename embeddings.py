...
class VectorDB:
    ...
    def search(self, query: str, k: int = 5, document_ids: List[str] = None) -> List[Dict]:
        """Search documents with improved filtering logic"""
        ...
        # If document_ids is specified, we need to search more broadly
        if document_ids:
            ...
            # Search more broadly when filtering by document IDs
            search_k = min(self.index.ntotal, max(k * 5, 20))  # Search at least 20 or k*5
            print(f"🔍 Expanding search to k={search_k} to find target documents")
        else:
            search_k = k
        ...
        try:
            ...
            for rank, (idx, distance) in enumerate(zip(indices[0], distances[0])):
                ...
                print(f"→ Match found! Rank {rank}: FAISS ID {idx} → MongoDB ID {doc_id}")
                ...
                try:
                    ...
                    # Look for both 'content' and 'text' fields
...
def check_document_in_index(self, document_id: str):
    """Check if a specific document is in the FAISS index"""
    print(f"🔍 Checking if document {document_id} is in FAISS index...")
    
    # Check if document exists in MongoDB
    mongo_doc = self.documents.find_one({"_id": ObjectId(document_id)})
    if not mongo_doc:
        print(f"❌ Document {document_id} not found in MongoDB")
        return False
    
    # Check if document is in FAISS mapping
    faiss_id = None
    for fid, doc_id in self.id_to_doc.items():
        if doc_id == document_id:
            faiss_id = fid
            break
    
    if faiss_id is None:
        print(f"❌ Document {document_id} not found in FAISS index")
        print("💡 You may need to rebuild the index or re-upload this document")
        return False
    
    print(f"✅ Document {document_id} found at FAISS ID {faiss_id}")
    return True
